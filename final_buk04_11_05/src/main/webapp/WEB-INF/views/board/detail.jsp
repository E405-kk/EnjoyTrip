...
<%@ include file="/WEB-INF/views/header.jsp" %>
	...
	<script>
      document.querySelector("#btn-list").addEventListener("click", function () {
        location.href = "${ root }/board?action=list&pgno=${pgno}&key=${key}&word=${word}";
      });
      document.querySelector("#btn-mv-modify").addEventListener("click", function () {
        location.href = "${ root }/board?action=mvmodify&articleno=${ article.articleNo }&pgno=${pgno}&key=${key}&word=${word}";
      });
      document.querySelector("#btn-delete").addEventListener("click", function(){
  		if (confirm("글을 삭제하시겠습니까?")){
  			location.href = "${ root }/board?action=remove&articleno=${ article.articleNo }&pgno=${pgno}&key=${key}&word=${word}";
  		} else {
  			return;
  		}
  	});
